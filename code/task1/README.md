...
tools.py file contain the python functions needed for each agent
prompt.py file contain the system prompt for the agent
...
For evaluating the API fetching agent, we extract this JSON and use the parameters within it (city for weather fetching and base/target for currency exchange) to re-query the APIs and verify if the API responses match the agent's responses using the evaluate_api_fetching_agent tool.
